...
namespace VectorMath
{
	...
	
	{
		...
		public unsafe AvxVec3(ReadOnlySpan<Vec3> origin, int offset) {
			fixed (Vec3* ptr = origin) 
			{
		...
		public unsafe void Store(Span<Vec3> destination, int offset) {
			fixed (Vec3* ptr = destination)
			{
				float* fp = (float*)&ptr[offset];
				Store8xyz(fp, x, y, z);
			}
		...
		public AvxVec3 Add(in AvxVec3 other) {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public AvxVec3 Multiply(in AvxVec3 other)
		{
			return new AvxVec3()
			{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public unsafe AvxVec3 Multiply(float scalar) {
